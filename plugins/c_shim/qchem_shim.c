 ...
 *
 * Limitations:
 *   - Does NOT support --embedcharge (needs xTB via Python).
 *     cli_amber.py falls back to Python shim when --embedcharge is active.
 */
...
static int evaluate_via_server(
    ...
    *ok = 0;
    ...
    /* Build request JSON (includes MM data so server can do embedcharge) */
    char *req = build_request_json(qm, nqm, charge, spinmult, mm_coords, mm_charges, nmm);
    if (!req) { fprintf(stderr, "[c-shim] malloc failed for request\n"); return -1; }

    /* Connect */
    int fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (fd < 0) { fprintf(stderr, "[c-shim] socket: %s\n", strerror(errno)); free(req); return -1; }
    ...
    close(fd);
    if (!resp) { fprintf(stderr, "[c-shim] recv failed\n"); return -1; }

    /* Check status */
    const char *st = json_find_key(resp, "status");
    if (!st || *st != '"') {
    ...
    if (strcmp(status_str, "ok") != 0) {
        const char *msg = json_find_key(resp, "message");
        if (msg && *msg == '"') {
    ...
    /* Parse energy_ev */
    const char *ep = json_find_key(resp, "energy_ev");
    if (!ep) { fprintf(stderr, "[c-shim] No energy_ev in response\n"); free(resp); return -1; }
    *energy_ev = json_parse_number(&ep);

    /* Parse forces_ev_ang: [[fx,fy,fz], ...] */
    const char *fp = json_find_key(resp, "forces_ev_ang");
    if (!fp || *fp == 'n') {
        /* null or missing → zero forces (shouldn't happen with need_forces=true) */
        for (int i = 0; i < nqm; i++) { forces_ev_ang[i][0] = forces_ev_ang[i][1] = forces_ev_ang[i][2] = 0.0; }
    } else {
        /* Expect: [[n,n,n],[n,n,n],...] */
        fp = skip_ws(fp);
        if (*fp != '[') { fprintf(stderr, "[c-shim] Expected '[' for forces\n"); free(resp); return -1; }
        fp++;
        for (int i = 0; i < nqm; i++) {
            fp = skip_ws(fp);
            if (*fp == ',') fp++;
            fp = skip_ws(fp);
            if (*fp != '[') { fprintf(stderr, "[c-shim] Expected '[' for force[%d]\n", i); free(resp); return -1; }
            fp++;
            forces_ev_ang[i][0] = json_parse_number(&fp);
            fp = skip_ws(fp); if (*fp == ',') fp++; fp = skip_ws(fp);
            forces_ev_ang[i][1] = json_parse_number(&fp);
            fp = skip_ws(fp); if (*fp == ',') fp++; fp = skip_ws(fp);
            forces_ev_ang[i][2] = json_parse_number(&fp);
            fp = skip_ws(fp);
            if (*fp == ']') fp++;
        }
    }

    /* Parse forces_mm_ev_ang (optional, present when embedcharge active) */
    if (nmm > 0) {
        memset(forces_mm_ev_ang, 0, sizeof(double) * 3 * nmm);
        const char *mp = json_find_key(resp, "forces_mm_ev_ang");
        if (mp && *mp == '[') {
            mp++;
            for (int i = 0; i < nmm; i++) {
                mp = skip_ws(mp);
                ...
                if (*mp != '[') break;
                mp++;
                forces_mm_ev_ang[i][0] = json_parse_number(&mp);
                mp = skip_ws(mp); if (*mp == ',') mp++; mp = skip_ws(mp);
                forces_mm_ev_ang[i][1] = json_parse_number(&mp);
                mp = skip_ws(mp); if (*mp == ',') mp++; mp = skip_ws(mp);
                forces_mm_ev_ang[i][2] = json_parse_number(&mp);
                mp = skip_ws(mp);
                if (*mp == ']') mp++;
            }
        }
