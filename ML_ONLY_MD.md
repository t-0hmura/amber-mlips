...
Key settings: `ntb=1` (NVT), `ntc=1,ntf=1` (no SHAKE), `qmmask='@*'` (all atoms as QM → pure ML MD), `cut=3.0` (dummy), `qmcut=0.0` (must be 0; sander checks QM region + qmcut < box), `--uma-task omat` (PBC-aware model).
